...
bmap(struct inode *ip, uint bn)
{
  ...
  bn -= NINDIRECT;
  if(bn < NINDIRECT_2){
    uint index = (bn & 0xff00) >> 8;   // fetch index
    uint offset = (bn & 0xff);         // fetch offset
...
itrunc(struct inode *ip)
{
  ...
  // for my mp3 release first doubly-indirect
  if(ip->addrs[NDIRECT+1]){
    bp = bread(ip->dev, ip->addrs[NDIRECT+1]);
    a = (uint*)bp->data;
    for(k = 0; k < NINDIRECT; k++){
      if(a[k]){
        bp_2 = bread(ip->dev, a[k]);
        b = (uint *)bp_2->data;
  ...
  // for my mp3 release second doubly-indirect
...
struct inode*
find_target(char *path){
  char to_where[MAXPATH];
  struct inode *ip;
  if((ip = namei(path)) == 0){
    return 0;
  }
  ...
  if(ip->type == T_SYMLINK){
    ...
    while(ip->type == T_SYMLINK){
        ...
        if(readi(ip, 0, (uint64)to_where, 0, sizeof(to_where))<0){
          iunlockput(ip);
          return 0;
        }
