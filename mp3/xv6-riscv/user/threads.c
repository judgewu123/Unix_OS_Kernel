...
void schedule(void){
    ...
    // ... implement SJF here.
    if(is_thread_start == 0){
        struct thread * to_run = current_thread;
        struct thread * tmp = to_run->next;
        ...
        while(tmp->ID!=start){
            if(tmp->remain_execution_time < to_run->remain_execution_time){
                to_run = tmp;
            }else if(tmp->remain_execution_time == to_run->remain_execution_time){
                if(tmp->ID < to_run->ID){
                    to_run = tmp;
                }
            }
        ...
        return;
    }else if(current_thread->is_exited){
        struct thread * to_run = current_thread->next;
        struct thread * tmp = to_run->next;
        int start = current_thread->ID;
        while(tmp->ID!=start){
            if(tmp->remain_execution_time < to_run->remain_execution_time){
