...
sys_thrdstop(void)
{
  ...
  p->handler_set = 1;
  p->ticks_to_handler = delay;
  p->ticks_used = 0;
  p->handler_addr = handler;
  p->handler_arg = handler_arg;
  if(thrdstop_context_id == -1){
    for(int i = 0; i < MAX_THRD_NUM+1; i++)
      if(p->thrd_stop_context_set[i] == -1){
        thrdstop_context_id = i;
        break;
      }
  }
  p->thrd_stop_context_set[thrdstop_context_id] = thrdstop_context_id;
...
sys_cancelthrdstop(void)
{
  ...
  if(is_exit == 0 && thrdstop_context_id != -1){
    // store context
    p->thrdstop_context_array[thrdstop_context_id] = *(p->trapframe);
  }
  else if(is_exit != 0){
    p->thrd_stop_context_set[thrdstop_context_id] = -1;
  }
...
sys_thrdresume(void)
{
  ...
  struct proc *p = myproc();
  memmove((uint64 *)p->trapframe + 5, (uint64 *)&p->thrdstop_context_array[thrdstop_context_id]+5, sizeof(struct trapframe) - 5*sizeof(uint64));
  p->trapframe->epc = p->thrdstop_context_array[thrdstop_context_id].epc;
